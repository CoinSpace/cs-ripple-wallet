...
export default class RippleWallet extends CsWallet {
  ...
  async unalias(alias) {
    ...
    try {
      const { classicAddress, tag } = xAddressToClassicAddress(alias);
      return {
        address: classicAddress,
        alias,
        destinationTag: tag !== false ? `${tag}` : undefined,
      };
  ...
  async validateAddress({ address }) {
    ...
    if (!(isValidClassicAddress(address) || isValidXAddress(address))) {
  ...
  async estimateImport({ privateKey }) {
    ...
    if (this.#isActive === false && sendable < this.#minReserve) {
      throw new errors.MinimumReserveDestinationError(new Amount(this.#minReserve + fee, this.crypto.decimals));
    }
