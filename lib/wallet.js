...
Wallet.prototype.loadTxs = function(address, start) {
  return this.api.accounts.txs(address, start, this.txsPerPage);
};
...
Wallet.prototype.createTx = function(to, value, tag, invoiceId) {
  var payment = {
    source: {
...
Wallet.prototype.createImportTx = function(options) {
  var amount = Big(options.amount).minus(this.minReserve).minus(this.getDefaultFee());
  if (amount.lt(0)) {
