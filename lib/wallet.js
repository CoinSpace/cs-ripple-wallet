...
export default class RippleWallet extends CsWallet {
  ...
  async validateMeta({ address, destinationTag, invoiceId }) {
    super.validateMeta({ address });
    if (destinationTag !== undefined && !isUint32(destinationTag)) {
      throw new InvalidDestinationTagError(destinationTag);
    }
