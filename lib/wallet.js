...
Wallet.prototype.loadTxs = function() {
  ...
  return this.api.accounts.txs(that.addressString, that.txsCursor).then(function(data) {
    data.txs = data.txs.map(function(tx) {
      ...
      tx.isIncoming = tx.to === that.addressString && tx.from !== tx.to;
      return tx;
    });
...
Wallet.prototype.unlock = function(privateKey) {
  this.account.secret = privateKey;
  this.isLocked = false;
};
...
Wallet.prototype.getImportTxOptions = function(secret) {
  ...
  if (address === this.account.address) return Promise.reject(new Error('Private key equal wallet private key'));
  var that = this;
