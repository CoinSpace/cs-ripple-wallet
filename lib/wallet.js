...
export class InvalidTagError extends TypeError {
  ...
  constructor(tag, options) {
    super(`Invalid Tag: '${tag}'`, options);
  }
}

// tecDST_TAG_NEEDED
export class DestinationTagNeededError extends TypeError {
  name = 'DestinationTagNeededError';
  constructor(message, options) {
    super(message || 'Destination Tag Needed', options);
  }
}

export class InvalidInvoiceIDError extends TypeError {
  name = 'InvalidInvoiceIDError';
  constructor(invoiceID, options) {
    super(`Invalid InvoiceID: '${invoiceID}'`, options);
  }
}
...
export class InvalidSecretError extends TypeError {
  ...
  constructor(message, options) {
    super(message || 'Invalid Secret', options);
  }
}
...
export default class RippleWallet extends CsWallet {
  ...
  async validateAmount({ address, amount }) {
    ...
    const destinationInfo = this.#getAccountInfo(address);
  ...
  async validateMeta({ address, tag, invoiceID }) {
    super.validateMeta({ address });
    if (tag !== undefined && !isUint32(tag)) {
      throw new InvalidTagError(tag);
    }
    if (invoiceID !== undefined && !isHash256(invoiceID)) {
      throw new InvalidInvoiceIDError(invoiceID);
    }
    return true;
  }
  ...
  async createTransaction({ address, amount, tag, invoiceID }, seed) {
  ...
  async #prepareImport(secret) {
    ...
    if (address === this.#address) {
      throw new InvalidSecretError('Private key equal wallet private key');
    }
    const info = await this.#getAccountInfo(address);
    const balance = this.#unitToAtom(info.balance);
    const fee = await this.#getMinerFee();
    const sendable = balance - this.#minReserve - fee;
    return {
      address,
      balance,
      sendable,
  ...
  async createImport({ secret }) {
    super.createImport();
    const { address, sendable, fee, sequence } = await this.#prepareImport(secret);
    const payment = {
      source: {
        address,
        maxAmount: {
    ...
    return this.#sendTransaction({ address, payment, instructions }, secret);
  }
